...
const walk=dir=>fs.readdirSync(path.join(root,dir),{withFileTypes:true}).flatMap(e=>{if(e.name.startsWith('.')||e.name==='node_modules'||e.name==='artifacts'||e.name==='__pycache__'||e.name.endsWith('.pyc'))return [];const p=path.posix.join(dir,e.name);return e.isDirectory()?walk(p):[p];});
