...
root.createCourseImageEditor=function(host,api){
  ...
  function setMode(mode){$('art-mode').value=mode;for(const key of ['pan','cart','reference'])$('art-action-'+key).setAttribute('aria-pressed',String(key===mode));$('art-route-finish').hidden=$('art-route-cancel').hidden=mode!=='route';message(mode==='pan'?'可拖动画面或已有点位。':mode==='cart'?'在图上点击放置球车，放下后可拖动。':mode==='reference'?'在图上点击放置目标点，放下后可拖动。':'在图上点击设置位置。');}
  ...
  let img=null,url=null,model=null,scale=1,x=0,y=0,baseScale=1,active=false,sequence=0,drag=null,route=[],hits=[],viewHole=null;
  ...
  function draw(){ctx.clearRect(0,0,canvas.width,canvas.height);ctx.fillStyle='#eee8dc';ctx.fillRect(0,0,canvas.width,canvas.height);hits=[];
    ...
    const live=api.live?.();$('art-live-state').textContent=live?(live.name+' · '+(R.geoToPixel(model,live.coordinate)?'图片范围内':'图片配准范围外，不显示假位置')+(api.hole().flag?' · 距旗 '+P.distanceMeters(live.coordinate,api.hole().flag).toFixed(1)+' m':'')):'未启用定位或路线模拟';
  ...
  $('art-from').onchange=draw;$('art-to').onchange=draw;$('art-labels').onchange=draw;$('art-coverage').onchange=draw;
  $('art-live').onclick=()=>api.startGPS();$('art-live-stop').onclick=()=>api.stopGPS();$('art-sim').onclick=()=>api.startSimulation();$('art-sim-stop').onclick=()=>api.stopSimulation();
  ...
  return {open,close,refresh:()=>{if(active)draw();}};
};
})(window);
