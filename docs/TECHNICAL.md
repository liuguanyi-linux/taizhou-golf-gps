...
新增 `course-workspace/gnss-session.js`，用会话代号隔离停止／重启后的迟到回调，并在权限拒绝、过期及质量不达标时清除当前定位。真实定位与模拟路线显式区分；GPS 不可用时不自动回退旧模拟点或静态目录点。测距同时展示起点与终点精度声明，未知则明确标识。新增 9 项自动测试；浏览器验证地图与高清图同点距离一致、停止模拟清空距离以及保存刷新坐标不变。详见仓库 `docs/UPDATE-2026-09-18.md`，本次没有新增外业精度真值。

## 2026-09-17 发布后精度检查更新

新增 `course-workspace/gnss-quality.js`：浏览器实时定位质量门槛、RTK 固定解 JSON 快照校验、原始坐标与设备精度元数据保留；高清图同步显示定位质量，过期位置不冒充实时 GPS。配准增加重复检验点排除和留一交叉检验。手动修改设备点后清除原设备精度声明。详见 [精度输入与验证说明](PRECISION-INPUT.md)。此更新提高检查与接入能力，没有新的外业真值，不宣称现场精度提升到指定米级或厘米级。
...
分发方式：公开 GitHub 仓库、公开下载包和 GitHub Pages 在线页面，无需登录。

本文根据现有前端源代码、静态数据及 2026-09-07 验收记录编写。
在线入口为 https://liuguanyi-linux.github.io/taizhou-golf-gps/ 。公开部署不表示已连接客户车载设备或完成现场测量验收。
...
2026-09-08 起，直接打开 GitHub Pages 在线入口即可使用，无需获取仓库访问权限。
仓库源码、Release 数据和图片均公开可下载。此前 2026-09-07 发布记录中的“私有”是当时状态，不再代表当前访问限制。
也可下载后在自己的电脑启动静态服务。网页编辑只保存到当前浏览器，不会公开写回仓库。
公开下载不提供副本追回或访问控制；不要提交含个人行踪的真实车辆记录到公开仓库。
...
当前公开静态版本已直接使用包内 JSON，并在 README 说明这一适配。
...
当前 GitHub Pages 是公开静态托管，没有登录或授权名单，也没有业务后端。
浏览器真实定位仍需用户授予位置权限，网络设备接入仍需客户实现。
如以后恢复朋友专用访问，须另外配置网站访问控制，不能只改变仓库可见性。
