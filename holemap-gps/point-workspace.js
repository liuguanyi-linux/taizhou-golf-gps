  ...
  function normalizePoint(input, options) {
    ...
    return {
      point: {
        ...
        accuracy_m: accuracy,
        ...Object.fromEntries(['accuracy_confidence','fix_type','correction_age_s','receiver'].filter(k=>input[k]!=null).map(k=>[k,input[k]])),
    ...
    for (const hole of course?.holes || []) {
      ...
      function add(localId, type, name, rawCoord, metadata) {
        ...
        points.push({
          ...
          ...Object.fromEntries(['accuracy_confidence','fix_type','correction_age_s','receiver'].filter(k=>props[k]!=null).map(k=>[k,props[k]])),
